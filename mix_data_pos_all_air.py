...
def pre_reshape(df: pd.DataFrame) -> pd.DataFrame:
    """数据时间尺度上变化，模拟频移
    ...
    # 频移。参考三个测试集最大值
    df_scaled["TOA"] *= np.clip(np.random.normal(1, 0.2), 0.75, 1.5)
    ...
    df_scaled["TOA"] += np.clip(np.random.exponential(1), 0, cut_len) # [hard]

    return df_scaled


def to_dict(df: pd.DataFrame, reshap=False):
    """数据转换为字典索引
    - 除以的超参数意义为「数字化单位」    
    reshap 需要有真实的 "TAG" 列
    """
    ...
    if reshap:
        for i in range(TAG_LEN):  # 需要有真实的 "TAG" 列
            ...
            di = df[df["TAG"] == i+1]["RF"]
            df.loc[df["TAG"] == i+1, "RF"] = np.random.uniform(0.2, 2) * (di - di.mean()) + di.mean() + \
                np.random.normal(0, 200)

            # PW 变换 (0.5, 2) 的缩放倍率参考 3 个测试集最大值
            di = df[df["TAG"] == i+1]["PW"]
            df.loc[df["TAG"] == i+1, "PW"] = np.random.uniform(0.5, 2) * (di - di.mean()) + di.mean() + \
                np.random.normal(0, 10)

            # DOA 变换
            df.loc[df["TAG"] == i+1, "DOA"] += np.random.uniform(0, 90)

        # 整体偏移 需要
        df["RF"] += np.random.uniform(-1000, 1000)
        
        df["PW"] += np.random.uniform(-10, 10)
        
        df["DOA"] += np.random.uniform(-60, 60) 
...
def _gen_mix_job(df_list: List[pd.DataFrame], size_2, if_time_reshap=False, test_df_split_list: List[List[pd.DataFrame]] = None, t=None):
    """生成混合窗口
    ...
    if if_time_reshap:
        df = pd.concat([pre_reshape(df_list_new[i]) for i in t], ignore_index=True)
    else:
...
def mix_data_gen(df_list: List[pd.DataFrame], size_1: int, size_2: int, n_jobs: int = -1, if_time_reshap=False, test_df_split_list=None, t=None):
    mix_windows = []

    # 并行生成混合窗口
    mixed_results = Parallel(n_jobs=n_jobs)(
        delayed(_gen_mix_job)(df_list, size_2, if_time_reshap, test_df_split_list, t) for _ in range(size_1)
    )
